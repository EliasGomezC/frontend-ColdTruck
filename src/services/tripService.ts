...
export const getInTransitTripCount = async (): Promise<{ activeTrips: number }> => {
    const { data } = await apiLocalHost.get('/trips/count/active');
    return data;
}

export const getScheduledTripCount = async (): Promise<{ scheduledTrips: number }> => {
    const { data } = await apiLocalHost.get('/trips/count/scheduled');
    return data;
}