import { EditIcon, Plus, SearchIcon } from "lucide-react"
...
const BoxList: React.FC = () => {
    ...
    const [selectedStatus, setSelectedStatus] = useState<string>('All') // Track the selected status
    ...
    // Filter boxes based on search query and selected status
    const filteredBoxes = boxes.filter((box) => {
        const query = searchQuery.toLowerCase()
        const statusMatch = selectedStatus === 'All' || box.status === selectedStatus

        return (
            (box._id.toString().includes(query) ||
            box.status.toLowerCase().includes(query) ||
            box.maxWeigth.toString().includes(query) ||
            getSensorForBox(box._id).includes(query)) && statusMatch
        )
    })
    ...
    return (
        <div>
            ...
            {/* SEARCH BAR */}
            <div className="w-full grid grid-cols-1 md:grid-cols-6 gap-3 mb-8 bg-white rounded-2xl shadow-xl border border-slate-200 p-6">
                <div className="col-span-5 relative">
                    ...
                    <input
                        type="text"
                        placeholder="Search by id, status, max weight, sensor..."
                ...
                <div className="col-span-1">
                    <select
                        ...
                        className="w-full pl-4 py-3 border border-slate-300 rounded-xl leading-5 bg-white placeholder-slate-500 focus:outline-none focus:placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                    >
            ...
            <div className="mb-4 text-sm text-slate-600">
                Showing {filteredBoxes.length} {filteredBoxes.length === 1 ? 'result' : 'results'}
            </div>
