...
const filteredTrucks = useMemo(() => {
    ...
    return trucks.filter((truck) => {
        ...
        const status = truck.status ? truck.status.toString().toLowerCase() : "";  // Asegúrate de que el status sea en minúsculas
        const truckId = String(truck._id).toLowerCase();  // Filtro por id
        ...
        return (
            ...
            status.includes(searchLower) ||  // Aquí se compara el estado en minúsculas
            truckId.includes(searchLower)  // Agregar el filtro por id
        );
    });
}, [trucks, searchTerm]);
        ...
        <div>
            <div className="flex justify-end flex-shrink-0 p-2">
                ...
                >
                    ...
                    />
                    {showTruckRegisterForm ? "Cancel" : "Add New Truck"}
                </button>
            </div>
            {/* Search Bar */}
            <div className="mb-8 bg-white rounded-2xl shadow-xl border border-slate-200 p-6">
                <div className="relative">
                    ...
                    <input
                        type="text"
                        placeholder="Search by id, license plates, brand, model, capacity, status..."
                        value={searchTerm}
                            ...
                            <tbody className="bg-white divide-y divide-slate-100">
                                {filteredTrucks.map((truck, index) => (
                                    <tr key={truck._id} className="hover:bg-slate-50 transition-colors duration-150">
                                        ...
                                        <td className="px-8 py-6 whitespace-nowrap">
                                            <span
                                                className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${statusStyles[truck.status] || statusStyles.Inactive   // fallback por si llega un estado no mapeado
                                                    }`}
                                            >
                                                {truck.status}
                                            </span>
