...
const CreateTripModal: React.FC<Props> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let message = "Error assigning trip. Please try again.";
      // El error de Axios viene con la propiedad response
      if (
        typeof error === "object" &&
        error !== null &&
        "response" in error &&
        ...
        if (data.error) {
          ...
          if (data.conflicto) {
            ...
            message += `\n(${new Date(c.scheduledDepartureDate).toLocaleString()} - ${new Date(c.scheduledArrivalDate).toLocaleString()})`;
          }
        }
      }
      toast.error(message, {
        style: {
          ...
          borderRadius: "12px",
          fontWeight: "500",
          whiteSpace: "pre-line", // Para saltos de línea
        },
      });
    } finally {
      setSubmitting(false);
  ...
  const SearchableSelect = ({
    ...
    return (
      <div className="relative" data-dropdown-id={placeholder}>
        <div
          className={`w-full px-4 py-3 border-2 rounded-xl bg-white transition-all duration-300 cursor-pointer flex items-center justify-between group hover:shadow-md ${show
        ...
        >
          <div className="flex items-center gap-3 flex-1">
            <Icon className={`h-5 w-5 transition-colors ${show ? "text-blue-500" : value ? "text-green-500" : "text-slate-400"
              }`} />
            <span className={`font-medium transition-colors ${displayValue ? "text-slate-900" : "text-slate-500"
              }`}>
              {displayValue || placeholder}
            </span>
          </div>
          <ChevronDown className={`h-5 w-5 text-slate-400 transition-all duration-300 ${show ? "rotate-180 text-blue-500" : ""
            }`} />
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <SearchableSelect
                      ...
                      displayField={(driver: UserType) => `#${driver.id} - ${driver.name} ${driver.lastName} ${driver.secondLastName}`}
                      idField="id"
                      icon={User}
                    />
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <SearchableSelect
                      data={trucks.map(t => ({
                        ...t,
                        displayLabel: `#${t._id} - ${t.plates}`
                      }))}
                      value={form.IDTruck}
                      onSelect={value =>
                        handleChange({ target: { name: "IDTruck", value } } as any)
                      }
