...
const ModalMoreTrip: React.FC<Props> = ({ isOpen, onClose, trip, onUpdated }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            // Cargar lecturas al abrir el modal
            fetchSensorReadings()

            // Configurar la actualización cada 20 segundos
            const intervalId = setInterval(() => {
                fetchSensorReadings()
            }, 20000) // 20 segundos

            // Limpiar intervalo cuando el modal se cierre
            return () => clearInterval(intervalId)
        }
    }, [isOpen, trip._id])

    // Función para calcular estadísticas de temperatura y humedad
    ...
    // Obtener las estadísticas de sensor
    const sensorStats = getSensorStats()

    // Ordenar las lecturas por fecha en orden descendente
                    ...
                    <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-6 text-white">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-indigo-100 text-sm">
                                Estimated distance: {(trip.estimatedDistance / 1000).toLocaleString()} km
                            </div>
                        </div>
                    </div>
                                ...
                                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-semibold text-gray-900">{(trip.estimatedDistance / 1000).toLocaleString()} km</p>
                                        </div>
                                    </div>
                                </div>
                                                    ...
                                                    <div key={r._id} className="flex items-center justify-between text-sm bg-white/50 rounded-lg p-2">
                                                        <span className="text-gray-600">
                                                            {new Date(r.dateTime).toLocaleString('en-US', {
                                                                year: 'numeric',  // Incluir el año
                                                                month: '2-digit', // Mes en 2 dígitos
                                                                day: '2-digit',   // Día en 2 dígitos
                                                                hour: '2-digit',  // Hora en 2 dígitos
                                                                minute: '2-digit',// Minuto en 2 dígitos
                                                                second: '2-digit', // Si deseas mostrar los segundos también
                                                            })}
                                                        </span>
